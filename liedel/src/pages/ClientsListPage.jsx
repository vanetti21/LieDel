...
import { ArrowLeft, Search, Users, MapPin, ShoppingCart, Calendar } from "lucide-react";
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <Users size={22} className="text-indigo-600" /> Lista General de Clientes
              </h2>
              <p className="text-xs text-gray-500">
                Visualización detallada de compras acumuladas, gastos y última interacción comercial.
              </p>
            </div>
          ...
          {loading ? (
            <div className="text-center py-16">
              ...
              <p className="text-indigo-600 text-sm font-medium animate-pulse">Cargando base de datos de clientes...</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-gray-100 font-mono text-gray-600">
                  {filteredClients.length === 0 ? (
                    ...
                    filteredClients.map((c, idx) => (
                      <tr 
                        key={idx} 
                        onDoubleClick={() => navigate(`/clients/detail/${c.Id_cliente}`)} 
                        className="hover:bg-indigo-50/50 cursor-pointer transition-colors"
                        title="Haz doble clic para ver el detalle del cliente"
                      >
                        <td className="p-3 font-sans text-gray-900 font-bold">{c.cliente}</td>
                        ...
                        <td className="p-3 text-right text-emerald-600 font-bold font-mono text-sm">
                          ${Number(c.total_gastado).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                        </td>
                        ...
                        <td className="p-3 text-center font-sans text-gray-500">
                          <span className="inline-flex items-center gap-1">
                            ...
                            {c.ultima_compra ? new Date(c.ultima_compra).toLocaleDateString() : "Sin compras"}
                          </span>
                        </td>
