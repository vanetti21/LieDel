  ...
  return (
    <div className="flex-1 overflow-auto relative z-10 font-sans">
      <main className="max-w-7xl mx-auto py-8 px-4 lg:px-8 space-y-8">
        ...
        <div 
          className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border border-gray-200 p-5 rounded-xl"
        ...
        >
          <div>
            ...
            <p className="text-xs text-gray-500">
              Análisis del valor financiero aportado por rango de fechas y segmentación de cartera.
            </p>
          </div>
        ...
        {loading && (
          <div className="text-center py-20 rounded-xl border border-gray-200" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
            ...
            <p className="text-indigo-600 font-medium text-sm">
              Extrayendo transacciones y calculando flujos de consumo comercial...
            </p>
          </div>
        )}
        ...
        {data && !loading && (
          <div id="reporteClientesCanvas" className="space-y-8 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
			  ...
			  {/* NUEVO: CICLO PROMEDIO ENTRE COMPRAS */}
				<div className="p-4 rounded-xl border border-gray-200 flex items-center gap-3 shadow-sm bg-[rgb(240,243,249)]">
					<div className="p-3 bg-blue-100 text-blue-600 rounded-lg"><Repeat size={20} /></div>
					<div>
					<span className="text-[10px] uppercase tracking-wider text-gray-400 font-bold block">Ciclo Recompra</span>
					<span className="text-lg font-black text-blue-600 font-mono">
						{data.ciclo_dias_promedio || 0} Días
					</span>
					<p className="text-[9px] text-gray-500 mt-0.5">Frecuencia entre compras.</p>
					</div>
				</div>
            </div>
			...
			<div className="lg:col-span-1 p-5 rounded-xl border border-gray-200 flex flex-col justify-between bg-[rgb(240,243,249)]">
				<div>
				<h3 className="text-sm font-bold mb-1 text-gray-900 flex items-center gap-2">
					<Award size={16} className="text-indigo-600"/> Cartera por Segmento
				</h3>
				<p className="text-[11px] text-gray-500 mb-3">División por volumen de capital inyectado.</p>
				</div>
				...
				<ResponsiveContainer width="100%" height="100%">
					...
					<Tooltip formatter={(value) => [`$${Number(value).toLocaleString('en-US', { minimumFractionDigits: 2 })}`, 'Total Aportado']} />
					</PieChart>
				</ResponsiveContainer>
				</div>
				...
				<table className="w-full text-left text-[11px]">
					...
					{data.segmentacion.map((seg, idx) => (
						...
						<td className="p-2 font-sans font-semibold flex items-center gap-1.5 text-gray-800">
							...
							/>
							<span className="truncate max-w-[90px]" title={seg.segmento}>
                          ...
                          <tr key={idx} className="hover:bg-gray-50">
                            <td className="p-2.5 pl-4 font-sans text-gray-900 font-semibold flex flex-col">
                              ...
                              <span className="text-[10px] text-gray-400 font-mono flex items-center gap-1">
                                <Phone size={10}/> {cl.Contacto_telefono || cl.Telefono || "S/N"}
                              </span>
                            </td>
