...
import { 
  UserCheck, UsersIcon, UserX, Calendar, RefreshCw, 
  DollarSign, Briefcase, TrendingUp, FileText, Percent, 
  Download, UserPlus, Award, Building2 , PieChart as PieIcon, BarChart2 
} from "lucide-react";
...
import { 
  ...
  CartesianGrid, BarChart, Bar, Tooltip, Legend, PieChart, Pie, Cell
} from "recharts";
...
const UsersPage = () => {
  ...
  try {
    ...
    const chartContratacionesEl = document.querySelector("#chart-contrataciones");
    const chartCargosEl = document.querySelector("#chart-distribucion-cargos");
    ...
    if (chartContratacionesEl) {
      const canvasContrataciones = await html2canvas(chartContratacionesEl, { scale: 2 });
  ...
  const PALETA_CARGOS = ['#6366F1', '#10B981', '#dc9213', '#EC4899', '#8B5CF6', '#3B82F6'];
        ...
        <div className="p-6 bg-white rounded-2xl text-gray-800 font-sans shadow-sm border border-gray-200">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8 border border-gray-200 p-5 rounded-xl" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
            <div>
              ...
              <p className="text-xs text-gray-500">Evaluación de metas comerciales y métricas operativas por rango de fecha y sucursal.</p>
            </div>
            ...
            <button onClick={fetchEmpleados} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-all active:scale-95 w-full">
          ...
          {reporteData && !loading && (
            <div id="reporteEmpleadosCanvas" className="space-y-8 bg-white p-6 rounded-2xl border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-5 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  ...
                  <div>
                    ...
                    <span className="text-xl font-black text-green-600 font-mono">${reporteData.total_recaudado.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                ...
                <div className="p-5 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  ...
                  <div>
                    ...
                    <span className="text-xl font-black text-indigo-600 font-mono">{reporteData.total_despachado.toLocaleString()} u.</span>
                ...
                <div className="p-5 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  ...
                  <div>
                    ...
                    <span className="text-xl font-black text-amber-600 font-mono">{reporteData.empleados_activos} Colab.</span>
                ...
                <div className="p-5 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  ...
                  <div>
                    ...
                    <span className="text-xl font-black text-purple-600 font-mono">${reporteData.rendimiento_medio_empleado.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div id="chart-evolucion-ventas" className="p-5 rounded-xl border border-gray-200" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  <h3 className="text-sm font-bold mb-3 text-gray-900 flex items-center gap-2">
                    <TrendingUp size={16} className="text-indigo-600"/> Evolución Mensual de Carga Operativa en Ventas
                  </h3>
                ...
                <div id="chart-contrataciones" className="p-5 rounded-xl border border-gray-200" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  <h3 className="text-sm font-bold mb-3 text-gray-900 flex items-center gap-2">
                    <UserPlus size={16} className="text-indigo-600"/> Evolución de Nuevas Contrataciones
                  </h3>
                  <ResponsiveContainer width="100%" height={180}>
                    <LineChart data={reporteData.evolucion_contrataciones || []}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                      <XAxis dataKey="periodo" tick={{ fontSize: 10 }} />
                      <YAxis allowDecimals={false} tick={{ fontSize: 10 }} />
                      <Tooltip formatter={(value) => [`${value} empleados`, 'Contratados']} />
                      <Line 
                        type="monotone" 
                ...
                <div className="rounded-xl border border-gray-200 p-5 overflow-hidden lg:col-span-2" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                      <PieIcon size={18} className="text-indigo-600" /> Distribución de Empleados por Cargo
                    </h3>
                    <span className="text-xs text-gray-500 font-medium">Composición del Personal</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center bg-white p-4 rounded-xl border border-gray-200">

                    {/* 1. Gráfico de Dona */}
                    <div id="chart-distribucion-cargos" className="h-[220px] w-full">
                      {reporteData.distribucion_cargos && reporteData.distribucion_cargos.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              paddingAngle={4}
                            >
                              {reporteData.distribucion_cargos.map((entry, index) => {
                                const PALETA_CARGOS = ['#6366F1', '#10B981', '#F59E0B', '#EC4899', '#8B5CF6', '#3B82F6'];
                                return <Cell key={`cell-${index}`} fill={PALETA_CARGOS[index % PALETA_CARGOS.length]} />;
                              })}
                            </Pie>
                      ...
                      <table className="w-full text-left text-xs text-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-100 font-mono text-gray-600">
                          {(() => {
                            ...
                            return reporteData.distribucion_cargos?.map((c, idx) => {
                              const porcentaje = totalCargos > 0
                                ...
                                <tr key={idx} className="hover:bg-gray-50">
                                  <td className="p-2 font-sans font-semibold text-gray-900 flex items-center gap-2">
                                    <span
                                      ...
                                      style={{ backgroundColor: PALETA_CARGOS[idx % PALETA_CARGOS.length] }}
                                    />
                                    {c.puesto}
                                  </td>
                                  <td className="p-2 text-center font-sans text-gray-600">{c.cantidad}</td>
              ...
              <div className="p-5 rounded-xl border border-gray-200" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                <h3 className="text-sm font-bold mb-4 text-gray-900 flex items-center gap-2">
                  <Award size={18} className="text-amber-500" /> Empleados con Mayor Recaudación Generada
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                  {(reporteData.top_generadores || []).map((top, index) => (
                    <div key={index} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between">
                      <div>
                        <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
                          Top #{index + 1}
                        </span>
                        <h4 className="text-xs font-bold text-gray-900 mt-2 whitespace-normal leading-tight">{top.empleado}</h4>
                        <p className="text-[10px] text-gray-400 whitespace-normal">{top.puesto || "Asesor"}</p>
                      </div>
                      <div className="mt-3 pt-2 border-t border-gray-100">
                        <span className="text-[9px] uppercase font-bold text-gray-400 block">Generado</span>
                        <span className="text-sm font-black text-green-600 font-mono">
                          ${top.total_generado.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                    </div>
                  ))}
                  ...
                  <table className="w-full text-left text-xs text-gray-700">
                    ...
                    <tbody className="divide-y divide-gray-200 font-mono text-gray-600">
                      {reporteData.tabla_empleados.length === 0 ? (
                        <tr>
                          <td colSpan="6" className="p-6 text-center text-gray-400 font-sans">❌ Ningún registro comercial vinculado a empleados en este rango de fechas / sucursal.</td>
                        </tr>
                      ) : (
                          ...
                          <tr key={idx} className="hover:bg-gray-50">
                            ...
                            <td className="p-3 text-center font-sans">
                              <span
                                ...
                                style={{
                                  backgroundColor: getColorPuesto(emp.puesto).bg,
                                  color: getColorPuesto(emp.puesto).text,
                                  borderColor: getColorPuesto(emp.puesto).border,
                                }}
                              >
                                {emp.puesto}
                              </span>
