        ...
        <div className="p-6 bg-white rounded-2xl text-gray-800 font-sans shadow-sm border border-gray-200">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8 border border-gray-200 p-5 rounded-xl" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
            ...
            >
                ...
                // Fondos sutiles alternados, manteniendo el texto SIEMPRE NEGRO (#000000)
          ...
          {reporteData && !loading && (
            <div id="reporteEmpleadosCanvas" className="space-y-8 bg-white p-6 rounded-2xl border border-gray-200">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* 1. DISTRIBUCIÓN POR CARGO (GRÁFICO DE PASTEL) */}
                <div className="p-5 rounded-xl border border-gray-200" style={{ backgroundColor: "rgb(240, 243, 249)" }}>
                    <h3 className="text-sm font-bold mb-3 text-gray-900 flex items-center gap-2">
                    <PieIcon size={16} className="text-indigo-600"/> Distribución de Empleados por Cargo
                    </h3>
                    ...
                    <PieChart>
                        ...
                        cy="45%"
                        outerRadius={75}
                        label={({ puesto, percent }) => `${puesto} (${(percent * 100).toFixed(0)}%)`}
                        labelLine={false}
                        >
                        {(reporteData.distribucion_cargos || []).map((entry, index) => {
                            const PALETA = ['#6366F1', '#10B981', '#F59E0B', '#EC4899', '#8B5CF6', '#3B82F6'];
                            return <Cell key={`cell-${index}`} fill={PALETA[index % PALETA.length]} />;
                        })}
                        </Pie>
                    </PieChart>
                    </ResponsiveContainer>
                  ...
                  {(reporteData.top_generadores || []).map((top, index) => (
                    <div key={index} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between">
                      <div>
                        ...
                        <h4 className="text-xs font-bold text-gray-900 mt-2 truncate">{top.empleado}</h4>
                        <p className="text-[10px] text-gray-400 truncate">{top.puesto || "Asesor"}</p>
                      </div>
