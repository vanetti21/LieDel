...
const styles = StyleSheet.create({
  ...
  chartTitle: { fontSize: 9, fontWeight: 'bold', color: '#374151', marginBottom: 4, textAlign: 'center' },
  kpiContainer: {
    flexDirection: 'row',
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>Auditoría Comercial y Retención de Clientes</Text>
          <Text style={styles.subtitle}>Período: {formatDate(fechaInicio)} al {formatDate(fechaFin)}</Text>
        </View>
        ...
        <View style={styles.kpiContainer}>
          <View style={styles.kpiBox}>
            ...
            <Text style={[styles.kpiValue, { color: '#16A34A' }]}>
              ${data?.facturacion_total?.toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </Text>
          ...
          <View style={styles.kpiBox}>
            ...
            <Text style={[styles.kpiValue, { color: '#7C3AED' }]}>
              ${data?.ticket_promedio_general?.toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </Text>
        ...
        {chartImages?.evolucionClientes && (
          <View style={{ marginBottom: 10 }} wrap={false}>
            <Image style={styles.fullWidthChartImage} src={chartImages.evolucionClientes} />
          </View>
        )}
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 10 }}>
          ...
          <View style={{ width: '35%' }}>
            <Text style={styles.sectionTitle}>Cartera por Segmento</Text>
            {chartImages?.segmentacion && (
              <View style={{ alignItems: 'center', marginBottom: 6 }} wrap={false}>
                <Image style={{ width: '75%', height: 100, objectFit: 'contain' }} src={chartImages.segmentacion} />
              </View>
            )}
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                <Text style={{ width: '45%' }}>Segmento</Text>
                <Text style={{ width: '20%', textAlign: 'center' }}>Clientes</Text>
                <Text style={{ width: '35%', textAlign: 'right' }}>Aportado</Text>
              </View>
              {data?.segmentacion?.map((seg, idx) => (
                <View key={idx} style={styles.tableRow} wrap={false}>
                  <View style={{ width: '45%', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{ fontWeight: 'bold', fontSize: 8 }}>{seg.segmento}</Text>
                  </View>
                  <Text style={{ width: '20%', textAlign: 'center' }}>{seg.cantidad_clientes}</Text>
                  <Text style={{ width: '35%', textAlign: 'right', color: '#16A34A', fontWeight: 'bold' }}>
                    ${Number(seg.aporte_financiero || 0).toLocaleString('en-US', { minimumFractionDigits: 0 })}
                  </Text>
                </View>
              ))}
            </View>
          ...
          {/* TOP CLIENTES */}
          <View style={{ width: '65%' }}>
            <Text style={styles.sectionTitle}>Ranking Top Clientes de Mayor Valor (CLV)</Text>
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                <Text style={{ width: '8%' }}>#</Text>
                <Text style={{ width: '37%' }}>Cliente</Text>
                <Text style={{ width: '20%', textAlign: 'center' }}>Frecuencia</Text>
                <Text style={{ width: '17%', textAlign: 'right' }}>Ticket Prom.</Text>
                <Text style={{ width: '18%', textAlign: 'right' }}>Total Aportado</Text>
              </View>
              {data?.top_clientes?.length > 0 ? (
                data.top_clientes.slice(0, 10).map((cl, idx) => (
                  <View key={idx} style={styles.tableRow} wrap={false}>
                    <Text style={{ width: '8%', color: '#94A3B8', fontWeight: 'bold' }}>{getMedalOrRank(idx)}</Text>
                    <View style={{ width: '37%' }}>
                    ...
                    <Text style={{ width: '20%', textAlign: 'center' }}>{cl.compras_realizadas} trans.</Text>
                    <Text style={{ width: '17%', textAlign: 'right', color: '#4F46E5', fontWeight: 'bold' }}>
                      ${cl.ticket_promedio?.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </Text>
                    <Text style={{ width: '18%', textAlign: 'right', color: '#16A34A', fontWeight: 'bold' }}>
