...
const CATEGORIAS = [
  ...
  },
  {
    ...
    hojas: [{ id: "empleados", label: "Ranking de Empleados" }],
  },
];
...
const ExcelExporter = ({ startDate, endDate }) => {
  ...
  const toggleTodaCategoria = (categoria) => {
    ...
    const update = {};
    categoria.hojas.forEach((h) => {
      update[h.id] = !todasMarcadas;
    });
      ...
      >
        ...
        <p className="text-sm text-gray-500">
          ...
          {totalSeleccionadas > 0 && (
            <span className="ml-2 font-semibold text-green-700">
              {totalSeleccionadas} hoja{totalSeleccionadas > 1 ? "s" : ""}{" "}
              seleccionada{totalSeleccionadas > 1 ? "s" : ""}
            </span>
          )}
        </p>
        ...
        <div className="space-y-2">
          {CATEGORIAS.map((categoria) => {
            ...
            const todasMarcadas = categoria.hojas.every(
              (h) => seleccionadas[h.id],
            );
            const algunaMarcada = categoria.hojas.some(
              (h) => seleccionadas[h.id],
            );
                ...
                <div className="flex items-center justify-between bg-white px-4 py-3">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      ref={(el) => {
                        if (el)
                          el.indeterminate = algunaMarcada && !todasMarcadas;
                      }}
