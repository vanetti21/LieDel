  ...
  const fetchInventario = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/reporte-inventario?fecha_inicio=${fechaInicio}&fecha_fin=${fechaFin}`,
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-200 pb-5 gap-4">
        <div>
          <h1 className="text-xl font-black text-gray-900 tracking-tight flex items-center gap-2">
            <Package className="text-indigo-600" size={22} /> Auditoría y
            Control de Inventarios
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Análisis predictivo de existencias, capital inmovilizado por almacén
            y alertas de reabastecimiento.
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm">
              ...
              <div>
                ...
                <h3 className="text-lg font-black text-gray-900">
                  {data.unidades_totales.toLocaleString()} u.
                </h3>
              </div>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm">
              ...
              <div>
                ...
                <h3 className="text-lg font-black text-gray-900">
                  $
                  {data.capital_inmovilizado.toLocaleString("en-US", {
          ...
          >
            <div className="p-3.5 bg-white/50 border-b border-gray-200 flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-gray-900">
                Distribución Física y Valorización por Almacén
              </h3>
            </div>
                ...
                <tbody className="divide-y divide-gray-100 font-sans text-gray-600 text-[11px]">
                  {data.inventario_almacenes.map((alm, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="p-2 pl-4 font-bold text-gray-900 flex items-center gap-1.5">
                        <Store size={12} className="text-gray-400" />{" "}
                        {alm.almacen}
                      </td>
                      <td className="p-2 text-gray-500 flex items-center gap-0.5">
                      ...
                      <td className="p-2 text-center font-mono font-bold text-gray-800">
                        {alm.unidades.toLocaleString()} u.
                      </td>
                      <td className="p-2 text-right font-mono font-black text-gray-900 pr-4">
                        $
                        {alm.capital_almacen.toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                    </tr>
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row items-center gap-6">
            <div className="w-full md:w-1/3 h-48 flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={data.distribucion_categorias}
                    cx="50%"
                    cy="50%"
                    innerRadius={55}
                    outerRadius={75}
                    paddingAngle={4}
                    dataKey="stock_categoria"
                  >
                ...
                <tbody className="divide-y divide-gray-100 font-sans">
                  {data.distribucion_categorias.map((cat, index) => (
                    <tr key={index} className="hover:bg-gray-50/50">
                      ...
                      <td className="p-2 text-center font-mono">
                        {cat.stock_categoria.toLocaleString()} u.
                      </td>
                      <td className="p-2 text-right font-mono font-bold text-gray-900">
                        $
                        {cat.inversion_categoria.toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                    </tr>
          ...
          >
            <div className="p-3.5 bg-white/50 border-b border-gray-200 flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-gray-900">
                Top 10 Artículos con Mayor Capital Inmovilizado
              </h3>
            </div>
            <div className="overflow-x-auto bg-white flex-grow">
              <table className="w-full text-left text-xs text-gray-700">
                ...
                <tbody className="divide-y divide-gray-100 font-sans text-gray-600 text-[11px]">
                  {data.top_inversion.map((item) => (
                    <tr key={item.Id_producto} className="hover:bg-gray-50">
                      <td className="p-2 pl-4 font-mono text-gray-400">
                        {item.Id_producto}
                      </td>
                      ...
                      <td className="p-2 text-right font-mono font-black text-indigo-950 pr-4">
                        $
          ...
          >
            <div className="p-3.5 bg-red-50/50 border-b border-red-100 flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-red-900">
                Auditoría Dinámica Predictiva (Stock ≤ Mínimo Dinámico en el
                Periodo)
              </h3>
            </div>
            <div className="overflow-x-auto bg-white flex-grow">
                ...
                <tbody className="divide-y divide-gray-100 font-mono text-gray-600 text-[11px]">
                  {data.lista_criticos.length === 0 ? (
                    <tr>
                      ...
                      >
                        🎉 Inventario balanceado. Ningún artículo requiere
                        reabastecimiento crítico en este rango temporal.
                      </td>
                    </tr>
                  ) : (
                    data.lista_criticos.map((prod) => (
                      <tr key={prod.Id_producto} className="hover:bg-red-50/30">
                        <td className="p-2 pl-4 font-sans font-bold text-gray-900">
                          {prod.producto}
                        </td>
                        <td className="p-2 text-center text-gray-700">
                          {prod.consumo_diario} u/día
                        </td>
                        <td className="p-2 text-center text-gray-600">
                          {prod.tiempo_entrega} días
                        </td>
                        <td className="p-2 text-center text-gray-400">
                          {prod.stock_seguridad} u.
                        </td>
                        <td className="p-2 text-center font-bold text-gray-800">
                          {prod.minimo} u.
                        </td>
                        <td className="p-2 text-center text-red-600 font-black bg-red-50/50">
                          {prod.stock} u.
                        </td>
                        <td className="p-2 pr-4 text-center">
                          <span
                            className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${prod.stock === 0 ? "bg-black text-white" : "bg-red-100 text-red-800"}`}
                          >
